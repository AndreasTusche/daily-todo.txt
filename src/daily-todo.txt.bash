...
FILE_TODO=("todo.txt" "todo_priv.txt")
FILE_DONE=("done.txt" "done_priv.txt")
FILE_RPRT=("report.txt" "report_priv.txt")
...
if [[ $optDependenciesGraph > 0 ]]; then
	...
	if [[ $? > 0 ]]; then 
		CMD_DOT='/drives/c/My\ Program\ Files/graphviz/bin/dot'
		$CMD_DOT -V >/dev/null  2>&1
		if [[ $? > 0 ]]; then 
			exit 1
		fi
...
function txt2csv {
	...
	gawk '                         
		...
		{print}
	' | #
	gawk -vFILETIME=$( stat -c "%Y" $1) -vFILETYPE=$2 '
		BEGIN {
			IGNORECASE=1
			OFS=","
			c=0                             # field counter
...
function virtualSort {
	cat $1 | #
	gawk ' # needs GNU awk time functions
		BEGIN {
			FS=","
			OFS=","
			CNT_FIELDS='${CNT_FIELDS}'
